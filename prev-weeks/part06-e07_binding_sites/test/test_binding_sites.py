...
class BindingSites(unittest.TestCase):
    ...
    def test_features_and_labels(self):
        A, y = get_features_and_labels("data.seq")
        n, m = A.shape
    ...
    def test_hamming1(self):
        with patch(ph("plt.show")) as show:
            acc = cluster_hamming("data.seq")
            self.assertAlmostEqual(acc, 0.9985, places=4, msg="Function cluster_hamming returned incorrect accuracy for file %s!" % "data.seq")
